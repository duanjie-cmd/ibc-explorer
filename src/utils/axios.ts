...
export const executeCancel = (useCount?: boolean, name?: string) => {
    if (name) {
        return (window as any)[name] && (window as any)[name]();
    } else if (useCount) {
...
export const setExecuteCancel = (useCount?: boolean, name?: string) => {
    return new CancelToken(function executor(c) {
        if (name) {
            return ((window as any)[name] = c);
        } else if (useCount) {
