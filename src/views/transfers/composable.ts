...
import { getIbcTxsVolumeAPI, getTxDetailsByTxHashAPI } from '@/api/transfers';
...
import {
    IIbcTx,
    IIbcTxCount,
    IRequestIbcTxsVolume,
    ITransfersQueryParams
...
export const useSubTitleFilter = (
    ...
    const getDisplaySubtitle = (
        ...
        const displayValued =
            !showDefault && valuedText !== ''
                ? formatBigNumber(valuedText, 0)
                : DEFAULT_DISPLAY_TEXT;
    ...
    watch(ibcStatisticsTxs, (newValue) => {
        if (Number(newValue?.tx_all?.count) <= TOTAL_BOUND) {
...
export const useQueryDatas = (
    ...
    const getIbcTxsVolume = async (params: IRequestIbcTxsVolume) => {
        let getIbcTxsVolumeAPIChain: string | undefined;
        if (params.chain) {
            getIbcTxsVolumeAPIChain = await ChainHelper.handleChainIdToChain(params.chain);
        }
        getIbcTxsVolumeAPI({
            ...params,
            chain: getIbcTxsVolumeAPIChain
        }).then((data) => {
            txsValue.value = data.data.txs_value;
        });
    };
...
export const useSelectedParams = (
    ...
    const chainData = computed(() => {
        const supportedChains = ibcChains.value?.all.filter(
            (item) => item.status !== CHAIN_STATUS.offline
        );
        return [
            {
                children: [
            ...
            {
                children: ChainHelper.sortArrsByNames(supportedChains || []).map((v) => ({
                    title: v.pretty_name,
                    id: v.pretty_name,
